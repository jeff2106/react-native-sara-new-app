...
import FormulaireCarteCredit from '@/Containers/AddCreditCard/sections/FormulaireCarteCredit'
import Recapitulatif from '@/Containers/AddCreditCard/sections/Recapitulatif'
...
const Wizard = ({ data }) => {
  ...
  const REG_CARTE_VISA = new RegExp(Config.REG_NUMERO_CARTE_VISA)
  const REG_CARTE_MASTERCARD = new RegExp(Config.REG_NUMERO_CARTE_MASTERCARD)
  const REG_NUMERO_CVV = new RegExp(Config.REG_NUMERO_CVV)
  const REG_DATE_LIMITE = new RegExp(Config.REG_DATE_LIMITE)
  ...
  const [carteCredit, setCarteCredit] = useState({})

  const onDataChange = (cle, value) => {
    const newCarteCredit = { ...carteCredit }
    newCarteCredit[cle] = value
    setCarteCredit(newCarteCredit)
  }

  const onMultipleDataChange = values => {
    const newCompteBancaire = { ...carteCredit }
    setCarteCredit({ ...newCompteBancaire, ...values })
  }
  ...
  const steps = [
    ...
    {
      title: 'Récapitulatif',
      component: <Recapitulatif data={carteCredit} />,
    },
    {
      icon: 'check-all',
      title: 'Terminer',
