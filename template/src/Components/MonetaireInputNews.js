  ...
  useEffect(() => {
    if (value?.length > 0) {
      if (
        (min !== undefined &&
          min !== null &&
          parseInt(value[0]?.r_valeur, 10) < min) ||
        (min !== undefined &&
          min !== null &&
          parseInt(value[0]?.r_valeur, 10) > max)
      ) {
        setError(true)
        ...
        {!disabled && (
          <TextInputMask
            ...
            onEndEditing={() => {
              setFocused(false)
              onChangeText(
                value !== null && value[0]?.r_valeur.replace(/\s/g, ''),
              )
            }}
            ...
            type={'money'}
            options={MASK}
            value={value[0]?.r_valeur}
            onChangeText={val => {
              onChangeText(val.replace(/\s/g, ''))
            }}
            textAlign={'right'}
          />
        )}
