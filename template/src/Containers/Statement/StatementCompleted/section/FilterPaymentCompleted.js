...
import {
  ...
  ButtonPaiement,
  SearchTextInput,
} from '@/Containers/Statement/StatementCompleted/Components'
...
const FilterPaymentCompleted = ({ navigation, onChange, onChangeText }) => {
  ...
  //End
  const OldDate = moment(new Date()).format('YYYY-MM-DD')
  ...
  useEffect(() => {
    if (Date_obj?.datetype_1 !== '' && Date_obj.datetype_2 !== '') {
      ...
      if (Date_obj?.datetype_1 > OldDate) {
        alert('Les champs ont des dates incorrectes')
      } else {
        onChange({
          DateObj: Date_obj,
  ...
  /*
            ...
            {Refer == 'datetype_1' && moment(date ?? new Date()).format('YYYY')}
...
export default FilterPaymentCompleted
