...
const RegistrationExpertComptableFormDownloadScreen = props => {
  const dispatch = useDispatch()
  const { Images } = useTheme()

  const initDownload = () => {
    const name = uuid.v4()
    downloadFile(Config.EXPERT_COMPTABLE_PROCURATION_PDF_URL, {
      name,
      ext: 'pdf',
    })
  }
      ...
      <ScrollView style={tailwind('flex-1 p-4')}>
        <View style={[tailwind('items-center')]}>
          ...
          <View style={tailwind('flex-row mt-10 mb-12')}>
            <Button onPress={() => navigate('RegistrationExpertComptable')}>
...
export default RegistrationExpertComptableFormDownloadScreen
